...
class JB7_31Bookings_04Notifications_Service_Message_Created
{
	...
	/* TO ADMINS */
		if( ! $this->repo->isDisabled('email-booking-created-manager') ){
			$msg = $this->repo
				->findById( 'email-booking-created-manager' )
				;
	...
	/* TO CUSTOMER */
		if( ! $this->repo->isDisabled('email-booking-created-customer') ){
			if( property_exists($booking->details, 'email') ){
				$customer = new stdClass;
				$customer->email = $booking->details->email;

				$msg = $this->repo
					->findById( 'email-booking-created-customer' )
					;
				$subject = $this->parser->parse( $msg->subject, $booking );
				$body = $this->parser->parse( $msg->body, $booking );
				$msg = new JB7_04Notifications_Service_Message( $msg->id, $subject, $body );
